...
namespace aslam
{
  namespace cameras
  {
        ...
        std::vector<bool> &outCornerObserved) const
    {
      ...
      // std::cout << "tag size: " << detections.size() << std::endl;
   ...
   */
      ...
      for (iter = detections.begin(); iter != detections.end();)
      {
        ...
        for (int j = 0; j < 4; j++)
        {
          ...
          if (remove)
          {
            // std::cout << "remove due to border" << iter->id;
          }
        }
        ...
        if (iter->good != 1)
        {
          ...
          if (remove)
          {
            std::cout << "remove due to good is false" << iter->id;
          }
        }
      ...
      // {
      //   success = false;
      //   std::cout << "min " << _options.minTagsForValidObs << " " << detections.size() << std::endl;
      ...
      if (_options.showExtractionVideo)
      {
        ...
        for (unsigned i = 0; i < detections.size(); i++)
          for (unsigned j = 0; j < 4; j++)
          {
            ...
            if (!success)
              cv::putText(imageCopy1, "Detection failed! (frame not used) Grid Calibration",
                          cv::Point(50, 50), CV_FONT_HERSHEY_SIMPLEX, 0.8,
      ...
      for (unsigned int i = 0; i < detections.size(); i++)
      {
        ...
        unsigned int baseId = (int)(tagId / (col / 2)) * col * 2 + (tagId % (col / 2)) * 2;
        unsigned int pIdx[] = {baseId, baseId + 1, baseId + (unsigned int)col + 1, baseId + (unsigned int)col};
