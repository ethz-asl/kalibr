...
def plot2DCoordinateFrame(axis, T_0f, size=1, linewidth=3, color=None):
    """Plot a coordinate frame on a 3d axis. In the resulting plot,
    x = red, y = green, z = blue.
    ...
    X = numpy.append( [p_0[:,0].T] , [p_0[:,1].T], axis=0 )
    Y = numpy.append( [p_0[:,0].T] , [p_0[:,2].T], axis=0 )
    axis.plot(X[:,0],X[:,1],X[:,2],'r-', linewidth=linewidth)
    axis.plot(Y[:,0],Y[:,1],Y[:,2],'g-', linewidth=linewidth)
